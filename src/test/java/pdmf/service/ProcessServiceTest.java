...
public class ProcessServiceTest extends TestHelper {
	...
	@Test
	public void getList() {
		List<ProcessRec> list = processService.list(version, productName, topicName);
	...
	public void getList_Fail2() {
		List<ProcessRec> list = processService.list(version, productName + "NO", topicName);
		Assert.assertEquals(0, list.size());
	}

	@Test
	public void get() {
		processService.remove(version, productName, topicName, processName, seq, "test");
		ProcessRec rec = processService.get(version, productName, topicName, processName, seq);
	...
	public void storeNew() {
		...
		ProcessRec fetched = processService.get(version, productName, topicName, processName, seq);
	...
	public void storeExisting() {
		...
		fetched = processService.get(version, productName, topicName, processName, seq);
	...
	public void updatedByAnotherUser() {
		...
		ProcessRec fetched1 = processService.get(version, productName, topicName, processName, seq);
		fetched1.description = "CHANGED1";

		ProcessRec fetched2 = processService.get(version, productName, topicName, processName, seq);
